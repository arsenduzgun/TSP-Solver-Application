...
public class DivideAndConquer{
    	...
    	if(xCoordMaxDistBetweenLocations > yCoordMaxDistBetweenLocations) {
    		Collections.sort(locationList, new Comparator<Location>() {
    			...
    			public int compare(Location firstHalfLocation, Location secondHalfLocation) {
    				if(firstHalfLocation.xCoordinate > secondHalfLocation.xCoordinate) {
    					return 1;
    				}
    				return -1;
    			}
    		...
    		Collections.sort(locationList, new Comparator<Location>() {
    			@Override
    			public int compare(Location firstHalfLocation, Location secondHalfLocation) {
    				if(firstHalfLocation.yCoordinate > secondHalfLocation.yCoordinate) {
    					return 1;
    				}
    				return -1;
    			}
    ...
    public Location[] mergePaths(Location[] firstPath, Location[] secondPath, int l, int mid, int r){
    	...
    	for(int i = 0; i < firstPathArrayLength; i++) {
    		...
    		for(int j = 0; j < secondPathArrayLength; j++){
        		...
        		for(int m = 0; m < 2; m++) {
        			for(int n = 0; n < 2; n++) {
        				...
        				pathsMerged.add(temp1.get(m).get(0));
        				...
        				if(distance < shortestMergedDistance) {
        					...
        					for(Location location: pathsMerged) {
        						shortestMergedPath.add(location);
        					}
        				}
        				pathsMerged.clear();
        			}
    		...
    		pathFirstHalf.add(pathFirstHalf.get(0));
    		pathFirstHalf.remove(0);
    	}
    	int counter = 0;
    	for(int i = l; i <= r; i++) {
